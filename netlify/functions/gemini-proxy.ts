...
// types.ts から型をインポートするためにパスを調整します。
// netlify/functions/gemini-proxy.ts から見て、ルートの types.ts は ../../types.ts になります。
import type { GeneratedReply, UserProfile, GeneratedQuoteComment } from '../../types';

const GEMINI_API_KEY = process.env.API_KEY; // Netlifyの環境変数から取得

if (!GEMINI_API_KEY) {
  // このログはNetlifyの関数ログに出力されます
  console.error("CRITICAL: GEMINI_API_KEY environment variable is not set in Netlify function.");
  // Netlify関数が初期化時に致命的なエラーで停止する方法は限定的です。
  // handler内でチェックしてエラーレスポンスを返すのが一般的です。
}

// AIクライアントの初期化はハンドラ外で行い、再利用する
let ai: GoogleGenAI | null = null;
if (GEMINI_API_KEY) {
  ai = new GoogleGenAI({ apiKey: GEMINI_API_KEY });
}

const MAX_AI_REPLY_LENGTH = 280; // services/geminiService.ts と同じ値を設定

// services/geminiService.ts から持ってきたレアキャラクター定義
const RARE_CHARACTERS = [
    { id: 'tadasumen', name: 'ただすめん', usernameForPrompt: '@tadasumen', description: 'ごく普通の男性ですが、発言の最後に「！！！」や「！！！！！！！」のように複数の感嘆符を必ず付けます（例：「それは最高ですね！！！」「楽しみ！！！！！！！」）。感嘆符以外の口調は自然で標準的です。特に強い個性や趣味を前面に出すタイプではありません。', avatarDescription: '明るいオレンジ色系のシンプルなシルエットアイコン。', exampleReply: '「それ、すごくいいですね！！！！！！！」' },
    { id: 'kenta_niisan', name: 'ケンタ兄さん', usernameForPrompt: '@kenta_b', description: '自称「みんなのケンタ兄さん」。非常にエネルギッシュでフレンドリー、そして少し暑苦しいほどのポジティブさを持つ筋肉質の男性です。「やぁ！ケンタ兄さんだよ！」「今日も一日、元気もりもりで行こうじゃないか！」「その調子だ、素晴らしいぞ！」のような、元気な挨拶や励ましの言葉を頻繁に使います。絵文字はあまり使わず、言葉の勢いで表現するタイプです。少し大げさな表現を好みます。', avatarDescription: '鮮やかな赤色系のシンプルなシルエットアイコン。', exampleReply: '「やぁ！ケンタ兄さんだよ！そのアイデア、実に素晴らしいじゃないか！元気もりもりで応援するぞ！」' }
];
...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  if (!ai) {
    console.error("Netlify Function: Gemini AI SDK not initialized. API_KEY missing in function environment.");
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "AI Service Error: API key not configured on the server." }),
  ...
  try {
    ...
    const {
      action, // 'generateReplies' or 'generateQuoteComment'
      postText,
      replyingAsUser,
      pastUserPostTexts,
      mainUserName,
      originalPostText // For quote retweets
    } = payload;
    ...
    // --- ここから services/geminiService.ts のプロンプト構築ロジックを移植 ---
    // action の値 ('generateReplies' or 'generateQuoteComment') と
    // 入力 (postText, replyingAsUser など) に基づいて、
    // systemInstructionText と userPromptText を構築します。
    // この部分は元の services/geminiService.ts のロジックを正確に再現する必要があります。

    // --- generatePositiveReplies相当のロジック ---
    if (action === 'generateReplies') {
        ...
        if (replyingAsUser && replyingAsUser.initialReplyText) {
            systemInstructionText = `あなたは、ソーシャルメディアユーザー「${replyingAsUser.name}」になりきるAIです。
...
あなたの応答は必ずJSONオブジェクトでなければなりません。`;
            userPromptText = `あなたは「${replyingAsUser.name}」です。
...
`;
            ...
            systemInstructionText = `あなたは、ユーザーの投稿に対して、複数の、極めて多様な個性を持つ日本のソーシャルメディアユーザーになりきり、肯定的で、ユーザーの気持ちに自然と寄り添う思いやりのある返信を日本語で生成するAIです。
...
あなたの応答は必ずJSON配列でなければなりません。`;
            userPromptText = `ユーザーの投稿: "${postText}"
...
返信テキストには、投稿内容への共感や称賛、ユーザーの気持ちに寄り添う言葉、または関連する優しい知識の共有などを盛り込み、キャラクターの個性（簡潔さ、饒舌さ、絵文字の使用有無、方言、句読点の使い方、言葉遣いの丁寧さの度合いなど）を反映した多様な口調や表現を適切に使ってください。常に投稿者を肯定し、安心感を与える内容にしてください。`;
    ...
    // --- generateQuoteRetweetComment相当のロジック ---
        ...
        systemInstructionText = `あなたは、架空の日本のソーシャルメディアユーザーとして、他のユーザーの投稿に対して「引用リポスト」の形で、その投稿の魅力や価値、あるいは投稿者の感情に寄り添った思慮深いコメントを生成するAIです。
...
- 「静かな支持者（例: 図書館の片隅の'シズカ'、縁の下の力持ち'マモル'）」