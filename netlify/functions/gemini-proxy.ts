...
import type { GeneratedReply, UserProfile, GeneratedQuoteComment, GeminiProxyRequest } from '../../types'; // 型定義をインポート

const MAX_AI_REPLY_LENGTH = 280;
...
if (apiKey) {
  ...
  console.warn("API_KEY 環境変数がNetlify環境で設定されていません。Gemini APIの呼び出しは失敗します。");
}
...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  if (!ai) {
    return {
      statusCode: 500,
      body: JSON.stringify({ message: "Gemini AI SDKがサーバー側で初期化されていません。APIキーが設定されているか確認してください。" }),
  ...
  let requestBody: GeminiProxyRequest;
  try {
    if (!event.body) {
    ...
    return {
      statusCode: 400,
      body: JSON.stringify({ message: 'Invalid JSON in request body', error: (error as Error).message }),
      headers: { 'Content-Type': 'application/json' },
    };
  }
  ...
  try {
    if (action === 'generateReplies') {
      const { postText, replyingAsUser, pastUserPostTexts, mainUserName } = requestBody as Extract<GeminiProxyRequest, {action: 'generateReplies'}>;
      
      let systemInstruction: string;
      let userPrompt: string;
      // START: Copied and adapted prompt logic from original geminiService.ts for generatePositiveReplies
    ...
    返信テキストには、投稿内容への共感や称賛、ユーザーの気持ちに寄り添う言葉、または関連する優しい知識の共有などを盛り込み、キャラクターの個性（簡潔さ、饒舌さ、絵文字の使用有無、方言、句読点の使い方、言葉遣いの丁寧さの度合いなど）を反映した多様な口調や表現を適切に使ってください。常に投稿者を肯定し、安心感を与える内容にしてください。`;
      ...
      // END: Copied and adapted prompt logic for generatePositiveReplies
      ...
      const response: GenerateContentResponse = await ai.models.generateContent({
        ...
        config: {
          ...
          temperature: replyingAsUser ? 0.75 : 0.95,
          topP: replyingAsUser ? 0.88 : 0.98,
          topK: replyingAsUser ? 35 : 60
        }
      });
      ...
      let resultData: GeneratedReply[];
      if (replyingAsUser) {
        if (typeof parsedData === 'object' && parsedData !== null && 'username' in parsedData && 'replyText' in parsedData) {
          resultData = [parsedData.username === replyingAsUser.name ? parsedData : { ...parsedData, username: replyingAsUser.name }] as GeneratedReply[];
           ...
           resultData = [parsedData[0].username === replyingAsUser.name ? parsedData[0] : { ...parsedData[0], username: replyingAsUser.name }] as GeneratedReply[];
        } else {
          console.error("Parsed JSON for single reply is not in expected format:", parsedData);
          resultData = [{ username: replyingAsUser.name, replyText: "AIの応答形式に問題があり、返信を正しく表示できませんでした。" }];
        }
      } else {
        if (Array.isArray(parsedData) && parsedData.every(item => typeof item === 'object' && item !== null && 'username' in item && 'replyText' in item)) {
            resultData = parsedData as GeneratedReply[];
        } else {
            console.error("Parsed JSON for multiple replies is not in expected array format:", parsedData);
            resultData = [];
            if (typeof parsedData === 'object' && parsedData !== null) {
                for (const key in parsedData) {
                    if (Array.isArray(parsedData[key]) && parsedData[key].every((item: any) => typeof item === 'object' && item !== null && 'username' in item && 'replyText' in item)) {
                        resultData = parsedData[key] as GeneratedReply[];
                        break;
                    }
                }
            }
             if (resultData.length === 0) { // Fallback if still empty
                 return {
                    statusCode: 500,
                    body: JSON.stringify({ message: 'AIからの応答が期待した形式ではありませんでした。' }),
                    headers: { 'Content-Type': 'application/json' },
                };
            }
        }
    ...
    } else if (action === 'generateQuoteComment') {
      const { originalPostText, mainUserName } = requestBody as Extract<GeminiProxyRequest, {action: 'generateQuoteComment'}>;

      // START: Copied and adapted prompt logic from original geminiService.ts for generateQuoteRetweetComment
    ...
    `;
      // END: Copied and adapted prompt logic for generateQuoteRetweetComment
      ...
      if (typeof parsedData === 'object' && parsedData !== null && 'username' in parsedData && 'commentText' in parsedData && typeof parsedData.username === 'string' && typeof parsedData.commentText === 'string') {
        ...
        return {
          statusCode: 200,
          body: JSON.stringify(parsedData as GeneratedQuoteComment),
          headers: { 'Content-Type': 'application/json' },
        };
      } else {
        console.error("Parsed JSON for quote comment is not in expected format:", parsedData);
        return {
          statusCode: 500,
          body: JSON.stringify({ message: 'AIからの引用コメントが期待した形式ではありませんでした。' }),
          headers: { 'Content-Type': 'application/json' },
        };
      }
    } else {
    ...
    // Check for API key related errors (this is a guess, actual error messages may vary)
    if (errorMessage.toLowerCase().includes("api key") || 
      ...
      return {
        ...
        body: JSON.stringify({ message: "APIキーが無効か、または正しく設定されていません。" }),
        headers: { 'Content-Type': 'application/json' },
      };
    }
    return {
      statusCode: 500,
      body: JSON.stringify({ message: `Gemini APIの呼び出し中にサーバーエラーが発生しました: ${errorMessage}` }),
      headers: { 'Content-Type': 'application/json' },
    };
  }
};
...
declare module '../../types' {
  export interface GeminiProxyRequestBase {
    mainUserName?: string;
  }

  export interface GenerateRepliesRequest extends GeminiProxyRequestBase {
  ...
  export interface GenerateQuoteCommentRequest extends GeminiProxyRequestBase {
    action: 'generateQuoteComment';
    originalPostText: string;
  }

  export type GeminiProxyRequest = GenerateRepliesRequest | GenerateQuoteCommentRequest;
}
